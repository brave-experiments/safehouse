...
| `safehouse/ironflow_policy.py` | `IronFlow` engine — all five gate methods, `declassify()`, `apply_bridge_field()`; `Role` enum (`ROUTING`/`CONTENT`) |
...
| `tracer.py` | All display logic; `DemoSpec`; `detect_pipeline`; `_PIPELINE_ENV`; `_PIPELINE_RESET_DS` |
...
All output from `safehouse/` goes through `trace.emit()`. `print()` belongs only in `safehouse_cli/cli.py` and `tracer.py`. Stray prints break the structured audit trail.
...
5. **`safehouse/driver.py`** — add `_handle_<tool_name>() -> tuple[str, dict]`; register in `_HANDLERS`; add entry to `_DRIVER_ROUTING_FIELDS` with the exact routing field names
6. **`tracer.py`** — add event handler(s) to the relevant `DemoSpec`; if this is a new pipeline type, update `detect_pipeline()` and add an entry to `_PIPELINE_ENV`
...
| `"ROUTING CONFIDENTIALITY — routing field …"` | A `(T,priv)` value reached a ROUTING field | Routing addresses must be `(T,pub)` — call `declassify()` if needed |
| `"[Confinement] … confidentiality=priv"` | `(_, priv)` slot crossed the bridge without `declassify()` | Call `policy.declassify()` before `apply_bridge_field()` |
